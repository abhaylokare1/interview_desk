...
export default function InterviewForm({ interview, profile, publicSchedule = false }: { interview?: Interview; profile?: { name: string; contactNumber: string | null }; publicSchedule?: boolean }) {
  ...
  return <form ref={formRef} noValidate action={submit} onSubmit={validateRequiredFields} onInput={event => { const target = event.target as HTMLInputElement; if (target.name && target.value.trim()) setEmptyFields(previous => previous.filter(name => name !== target.name)); }} className="surface rounded-[1.35rem] p-4 sm:p-7">
    <input type="hidden" name="id" value={interview?.id ?? ""} />
    <div className="mb-7 flex items-center gap-3 border-b border-white/8 pb-5"><span className="grid size-10 place-items-center rounded-xl bg-gradient-to-br from-violet-500/25 to-sky-500/20 text-violet-200 ring-1 ring-violet-300/15">✦</span><div><h2 className="font-semibold tracking-tight text-white">Interview details</h2><p className="mt-0.5 text-xs text-slate-500">Fields marked * are required</p></div></div>
    ...
    <div className="mt-8 flex flex-col-reverse gap-3 border-t border-white/8 pt-5 sm:flex-row"><button type="button" onClick={() => router.back()} className="rounded-xl border border-white/12 bg-white/[.025] px-5 py-2.5 font-semibold text-slate-300 hover:bg-white/[.07]">Cancel</button>{whatsappUrl && <button type="button" onClick={() => { void sendToWhatsApp(); }} className="rounded-xl bg-[#25D366] px-5 py-2.5 text-center font-semibold text-slate-950 shadow-lg shadow-emerald-950/25 hover:bg-[#4be083]">Send to WhatsApp Group</button>}<button disabled={saving || !!whatsappUrl} className="rounded-xl bg-gradient-to-r from-violet-600 via-indigo-500 to-sky-500 px-5 py-2.5 font-semibold text-white shadow-lg shadow-violet-950/30 hover:brightness-110 disabled:opacity-60">{saving ? "Saving…" : publicSchedule ? "Schedule Interview" : interview ? "Update Interview" : "Save Interview"}</button></div>
